...
async def update_cinema(cinema_id: int, payload: UpdateCinema, cinema_client: CinemaServiceClient) -> CinemaRead:
    return await cinema_client.request("PATCH", f"/cinema/{cinema_id}", json=payload.model_dump())

@router.delete("/{cinema_id}", status_code=status.HTTP_204_NO_CONTENT, summary="Delete Cinema", dependencies=[admin])
async def delete_cinema_by_id(cinema_id: int, cinema_client: CinemaServiceClient) -> None:
