...
async def create_hall(payload: CreateHall, hall_client: CinemaServiceClient) -> HallRead:
    return await hall_client.request("POST", "/hall/", json=payload.model_dump())

@router.get("/{hall_id}", response_model=HallRead, status_code=status.HTTP_200_OK, summary="Hall", dependencies=[admin])
async def get_hall_by_id(hall_id: int, hall_client: CinemaServiceClient) -> HallRead:
...
async def get_hall_by_name(cinema_id: int, name: str, hall_client: CinemaServiceClient) -> HallRead:
...
async def update_hall(hall_id: int, payload: UpdateHall, hall_client: CinemaServiceClient) -> HallRead:
    return await hall_client.request("PATCH", f"/hall/{hall_id}", json=payload.model_dump())

@router.delete("/{hall_id}", status_code=status.HTTP_204_NO_CONTENT, summary="Delete Hall", dependencies=[admin])
async def delete_hall_by_id(hall_id: int, hall_client: CinemaServiceClient) -> None:
